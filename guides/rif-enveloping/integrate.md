...
Another option is to use Enveloping through a Relay Provider. A Relay Provider is a web3 provider and all transactions and calls are handled through it. The Relay Provider uses a Relay Client instance underground to interact with the Relay Server.
